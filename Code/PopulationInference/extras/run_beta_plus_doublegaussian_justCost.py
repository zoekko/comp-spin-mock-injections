...
import json
import sys
from posterior_extras import betaPlusDoubleGaussianJustCost
sys.path.append('..')
...
model_savename = model + f"_pop{pop}_{nevents}events_WIP3_just_cost" 
...
if nSteps>0: # if the run hasn't already finished
    ...
    sampler = mc.EnsembleSampler(
        nWalkers,
        dim,
        betaPlusDoubleGaussianJustCost, # model in posterior_extras.py
        args=[chi_hyperparams[pop],sampleDict,injectionDict,priorDict], # arguments passed to betaPlusDoubleGaussianJustCost
        threads=16,
...
results = {
    'mu1_cost':{'unprocessed':chainRaw[:,:,0].tolist(), 'processed':chainDownsampled[:,0].tolist()},
    'sigma1_cost':{'unprocessed':chainRaw[:,:,1].tolist(), 'processed':chainDownsampled[:,1].tolist()},
    'mu2_cost':{'unprocessed':chainRaw[:,:,2].tolist(), 'processed':chainDownsampled[:,2].tolist()},
    'sigma2_cost':{'unprocessed':chainRaw[:,:,3].tolist(), 'processed':chainDownsampled[:,3].tolist()},
    'MF_cost':{'unprocessed':chainRaw[:,:,4].tolist(), 'processed':chainDownsampled[:,4].tolist()},
