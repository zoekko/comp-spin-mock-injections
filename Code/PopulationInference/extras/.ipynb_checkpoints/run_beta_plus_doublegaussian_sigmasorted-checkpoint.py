...
import json
import sys
from posterior_extras import betaPlusDoubleGaussianSigmaSorted
sys.path.append('..')
...
model_savename = model + f"_pop{pop}_{nevents}events_WIP3_sigma_sorted" 
...
if len(old_chains)==0:
    ...
    hyperparams = { 
         ...
         # such that sigma1_cost < sigma2_cost 
        '1':[0.55, 0.26, 0.19, 0.18, 0.42, 0.75, 0.55, 0.96],  
        '2':[0.32, 0.16, 0.59, 0.40, 0.33, 0.64, 1-0.36, 0.96],
        '3':[0.19, 0.12, 0.98, 0.31, -0.98, 0.44, 1-0.26, 0.96]
    }
...
if nSteps>0: # if the run hasn't already finished
    ...
    sampler = mc.EnsembleSampler(
        nWalkers,
        dim,
        betaPlusDoubleGaussianSigmaSorted, # model in posterior_extras.py
