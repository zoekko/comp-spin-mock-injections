...
import json
import sys
from posterior_extras import betaPlusDoubleGaussianNoRedshift
sys.path.append('..')
...
model_savename = model + f"_pop{pop}_{nevents}events_noRedshift_fixedExtrinsic" 
...
if nSteps>0: # if the run hasn't already finished
    ...
    sampler = mc.EnsembleSampler(
        nWalkers,
        dim,
        betaPlusDoubleGaussianNoRedshift, # model in posterior_extras.py
