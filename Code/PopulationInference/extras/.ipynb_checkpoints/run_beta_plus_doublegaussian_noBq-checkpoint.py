...
import json
import sys
from posterior_extras import betaPlusDoubleGaussianNoBq
sys.path.append('..')
...
model_savename = model + f"_pop{pop}_{nevents}events_WIP3_no_Bq" 
...
if nSteps>0: # if the run hasn't already finished
    ...
    sampler = mc.EnsembleSampler(
        nWalkers,
        dim,
        betaPlusDoubleGaussianNoBq, # model in posterior_extras.py
