...
label = "job_{0:05d}_justSpin".format(args.job)
...
sampling_frequency = 2048. # TODO maybe change to 4096
fMin = 15.
...
while durationFlag==False:
    try:
        ...
        duration = duration + 0.5 # TODO: maybe change to duration = 2*duration
          
print('duration:',duration)
        
# For this analysis, we implement the standard precessing BBH priors defined in the prior_justSpin.prior
# for spin magnitudes and tilts and fix all other  parameters to truth
priors =  bilby.gw.prior.BBHPriorDict(directory+"Code/IndividualInference/prior_justSpin.prior")
extrinsic_params = [
    'ra','dec', 'geocent_time', 'phase', 'psi', 'luminosity_distance', 'theta_jn'
]
intrinsic_params = [
    'mass_1', 'mass_2', 'phi_12', 'phi_jl'
]
for param in np.concatenate([extrinsic_params,intrinsic_params]): 
